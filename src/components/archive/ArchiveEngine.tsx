...
export const ArchiveEngine = ({ tableName, archiveTitle, themeColor, logo, formats, AddModal, DetailsModal }: any) => {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white pb-32 pt-20">
      ...
      <main className="px-6 mt-10 max-w-[1800px] mx-auto w-full overflow-hidden text-center">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`relative z-0 grid grid-cols-${cols} md:grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4`}
          >
        ...
        {showFilters && (
          <>
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="fixed bottom-0 left-0 right-0 bg-zinc-900 rounded-t-[3rem] border-t border-white/10 p-8 pt-10 z-[220] shadow-2xl text-left">
              ...
              <button onClick={() => setShowFilters(false)} className="w-[calc(100%-2rem)] py-5 bg-white text-black rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-transform max-w-lg mx-auto block mb-6">Apply Filters</button>
            </motion.div>
          </>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence mode="wait">
        {showAddModal && <AddModal onClose={() => setShowAddModal(false)} onSuccess={fetchAlbums} discogsToken={discogsToken} />}
        {selectedAlbum && (
          <DetailsModal 
            album={selectedAlbum} 
