...
import { Plus, Settings2, Search as SearchIcon, Filter, Disc, BookmarkCheck, ArrowUpDown, X, ListMusic, Loader2 } from 'lucide-react';
...
export const ArchiveEngine = ({ tableName, archiveTitle, themeColor, logo, formats, AddModal, DetailsModal }: any) => {
  ...
  const processedAlbums = useMemo(() => {
    let result = [...albums];
    if (searchTerm) result = result.filter(a => a.title.toLowerCase().includes(searchTerm.toLowerCase()) || a.artist.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const lastAlbumRef = useCallback((node: HTMLDivElement) => {
    ...
    observer.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && visibleCount < processedAlbums.length) setVisibleCount(p => p + 40);
    });
    if (node) observer.current.observe(node);
  }, [visibleCount, processedAlbums.length]);

  const currentIndex = useMemo(() => processedAlbums.findIndex(a => a.id === selectedAlbum?.id), [processedAlbums, selectedAlbum]);
  const stats = useMemo(() => ({ total: albums.length, owned: albums.filter(a => a.status === 'MAM').length, wanted: albums.filter(a => a.status === 'SZUKAM').length }), [albums]);
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white pb-32">
      ...
      <header className="px-6 mt-16 space-y-8 max-w-[1800px] mx-auto w-full">
        ...
        <div className="relative">
          ...
          <input type="text" placeholder={`Szukaj w ${archiveTitle}...`} className="w-full bg-zinc-900/30 border border-white/5 rounded-[1.5rem] py-4 pl-12 pr-12 text-sm font-bold outline-none focus:bg-zinc-900/60 focus:border-brand/30 transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </header>

      <main className="px-6 mt-12 max-w-[1800px] mx-auto w-full">
        <AnimatePresence mode="wait">
          <motion.div key={tableName} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`grid grid-cols-${cols} md:grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4`}>
            {processedAlbums.slice(0, visibleCount).map((album, idx) => (
              <motion.div key={album.id} layoutId={album.id} ref={idx === visibleCount - 1 ? lastAlbumRef : null} onClick={() => setSelectedAlbum(album)} className="group relative aspect-square bg-zinc-900 rounded-xl overflow-hidden cursor-pointer shadow-xl active:scale-95 transition-transform">
