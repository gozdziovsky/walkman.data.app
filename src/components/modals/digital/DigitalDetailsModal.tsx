...
export const DigitalDetailsModal = ({ 
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-xl flex items-center justify-center p-0 md:p-6 lg:p-12" 
      onClick={onClose}
    >
      {/* Nawigacja Desktopowa */}
      {!isEdit && (
        <>
          {onPrev && <button onClick={(e) => { e.stopPropagation(); setDirection(-1); onPrev(); }} className="hidden lg:flex absolute left-8 p-5 text-white/10 hover:text-brand transition-all active:scale-90"><ChevronLeft size={64} /></button>}
          {onNext && <button onClick={(e) => { e.stopPropagation(); setDirection(1); onNext(); }} className="hidden lg:flex absolute right-8 p-5 text-white/10 hover:text-brand transition-all active:scale-90"><ChevronRight size={64} /></button>}
        </>
      )}
      ...
      <motion.div 
        ...
        className="bg-zinc-900 w-full max-w-6xl rounded-t-[2.5rem] md:rounded-[3rem] overflow-hidden flex flex-col md:flex-row h-[95vh] md:h-auto max-h-[90vh] shadow-2xl relative border border-white/5"
      ...
      >
              ...
              <motion.div key="c" className="w-full h-full relative">
                ...
                {!isEdit && (
                  <button 
                    ...
                    className="absolute bottom-6 left-6 p-3 bg-black/60 backdrop-blur-md rounded-full text-white/50 hover:text-brand hover:bg-black transition-all border border-white/10 active:scale-90"
                  >
                    <Edit3 size={18} />
                  </button>
                )}
                ...
                {hasTracks && (
                  <div onClick={() => setShowTracks(true)} className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center opacity-40 group-hover:opacity-100 transition-opacity animate-bounce cursor-pointer">
        ...
        {/* PRAWA STRONA: INFO (Z wewnętrznym scrollem) */}
        <div className="p-8 md:p-12 flex-1 overflow-y-auto no-scrollbar text-left bg-gradient-to-br from-zinc-900 to-black flex flex-col justify-center">
          <AnimatePresence mode="wait">
            {isEdit ? (
              <motion.div key="edit" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full">
                 ...
                 <div className="flex gap-3 pt-4">
                    <button onClick={handleUpdate} className="flex-[2] py-5 bg-brand text-black rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-all">
                      {loading ? 'Saving...' : 'Save Changes'}
                    </button>
                    <button onClick={() => setIsEdit(false)} className="flex-1 py-5 bg-zinc-800 rounded-2xl font-black uppercase text-[10px] tracking-widest active:scale-95 transition-all">Cancel</button>
                 </div>
              </motion.div>
            ) : (
              <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 w-full">
                <header>
                  <button 
                    onClick={() => onArtistClick(album.artist)} 
                    className="text-brand font-black uppercase text-[11px] tracking-tighter italic mb-3 hover:opacity-70 transition-opacity"
                  >
                    {album.artist}
                  </button>
                  <h2 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter leading-[0.9] mb-8 break-words text-white">
                    {album.title}
                  </h2>
                  <div className="flex flex-wrap gap-3">
                    <Badge icon={<Calendar size={12}/>} text={album.year?.toString() || 'N/A'} />
                    <Badge icon={<Disc size={12}/>} text={album.format} brand />
                    {/* POPRAWKA: Puste 0 nie będzie już widoczne */}
                ...
                <footer className="pt-10 border-t border-white/5 flex gap-6">
                    <button onClick={handleDelete} className="text-zinc-600 hover:text-red-500 transition-colors flex items-center gap-2 font-black uppercase text-[10px] tracking-widest active:scale-90">
...
const Badge = ({ icon, text, brand }: any) => (
  <span className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${brand ? 'bg-brand text-black shadow-lg shadow-brand/10' : 'bg-white/5 text-zinc-400 border border-white/5'}`}>
    {icon} {text}
  </span>
);

const ActionButton = ({ icon, text, primary, onClick }: any) => (
  <button onClick={onClick} className={`py-5 rounded-[2rem] font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl ${primary ? 'bg-white text-black hover:bg-brand' : 'bg-zinc-800 text-white hover:bg-zinc-700'}`}>
    {icon} {text}
  </button>
);
...
const FormInput = ({ label, value, onChange, type = "text" }: any) => (
  <div className="space-y-1 text-left">
    ...
    <input 
      type={type} 
      className="w-full bg-zinc-800 border border-white/5 rounded-xl px-5 py-4 text-sm font-bold text-white outline-none focus:border-brand/50 transition-all" 
      value={value} 
