    ...
    >
      {!isEdit && (
        <div className="hidden lg:contents">
          ...
          {onNext && <button onClick={(e) => { e.stopPropagation(); paginate(1, onNext); }} className="absolute right-8 top-1/2 -translate-y-1/2 p-5 text-white/5 hover:text-brand transition-all active:scale-90"><ChevronRight size={64} /></button>}
        </div>
      )}
      ...
      >
          ...
          <AnimatePresence mode="wait">
            {showTracks ? (
              ...
              <motion.div key="c" className="w-full h-full relative" onClick={() => hasTracks && setShowTracks(true)}>
                ...
                {hasTracks && (
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center opacity-40 group-hover:opacity-100 transition-opacity animate-bounce cursor-pointer">
        ...
        <div className="p-8 md:p-14 lg:p-20 flex-1 flex flex-col justify-between bg-gradient-to-br from-[#0e0e10] to-black min-h-0 overflow-hidden">
          <AnimatePresence mode="wait">
            {isEdit ? (
              <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 my-auto text-left w-full max-w-md mx-auto">
