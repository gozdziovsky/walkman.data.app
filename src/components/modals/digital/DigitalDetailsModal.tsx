...
export const DigitalDetailsModal = ({ album, onClose, onUpdateSuccess, onArtistClick, onNext, onPrev }: any) => {
  ...
  return (
    // Tło Modala (identyczne jak w Settings)
    <motion.div 
    ...
    >
      ...
      <motion.div 
        ...
        className="bg-[#0e0e10] w-full max-w-5xl h-[92vh] md:h-[85vh] rounded-t-[2.5rem] md:rounded-[3.5rem] flex flex-col md:flex-row overflow-hidden relative transform-gpu will-change-transform shadow-[0_0_80px_rgba(0,0,0,0.8)]"
      >
        ...
        {/* GŁÓWNA ZAWARTOŚĆ Z ANIMACJĄ PRZEJŚCIA MIĘDZY ALBUMAMI */}
                ...
                <AnimatePresence mode="wait">
                  {showTracks ? (
                    ...
                    <motion.div key="c" className="w-full h-full relative cursor-pointer" onClick={() => hasTracks && setShowTracks(true)}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setIsEdit(true); }} className="absolute bottom-4 left-4 md:bottom-6 md:left-6 p-4 bg-black/60 backdrop-blur-md rounded-2xl text-white/50 hover:text-brand transition-all border border-white/10 active:scale-90 shadow-2xl z-50">
                        <Edit3 size={18} />
                      </button>
                      {hasTracks && (
            ...
            <div className={`flex-1 flex flex-col min-h-0 bg-gradient-to-br from-[#0e0e10] to-black relative ${isEdit ? 'w-full' : ''}`}>
              ...
              {isEdit ? (
                ...
                <div className="flex flex-col w-full h-full">
                  ...
                  <div className="shrink-0 p-6 md:p-12 pt-6 bg-[#0e0e10] border-t border-white/5 z-30">
                ...
                /* ========================================== */
                <div className="flex-1 overflow-y-auto no-scrollbar p-8 md:p-14 lg:p-20 flex flex-col justify-between text-left relative">
                  <header className="pt-2 md:pt-0 shrink-0 pr-12 md:pr-0">
                    <div className="flex flex-wrap gap-x-3 gap-y-1 mb-6">
                      {artists.map((name, i) => (
                        <button key={i} onClick={() => onArtistClick(name)} className="text-brand font-black uppercase text-[11px] md:text-[12px] tracking-tighter italic hover:text-white transition-colors flex items-center">
                  ...
                  <div className="space-y-8 mt-12 shrink-0">
...
const Badge = ({ icon, text, brand, colorClass }: any) => (
  <span className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg transition-all
