          ...
          <AnimatePresence mode="wait">
            {showTracks ? (
              ...
              <motion.div key="c" className="w-full h-full relative" onClick={() => hasTracks && setShowTracks(true)}>
                ...
                {!isEdit && (
                  <button onClick={() => setIsEdit(true)} className="absolute bottom-6 left-6 p-4 bg-black/60 backdrop-blur-md rounded-2xl text-white/40 hover:text-brand hover:bg-black transition-all border border-white/10 active:scale-90 shadow-2xl"><Edit3 size={18} /></button>
                )}
                {hasTracks && (
