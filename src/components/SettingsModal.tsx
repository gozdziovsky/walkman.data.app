        ...
        <div className="space-y-6">
          <section>
            ...
            <div className="grid grid-cols-4 gap-2 bg-black/40 p-1.5 rounded-2xl border border-white/5">
              {[1, 2, 3, 4].map((num) => (
                <button key={num} onClick={() => { localStorage.setItem('walkman_cols', num.toString()); setCols(num); }} className={`py-3 rounded-xl text-xs font-black transition-all ${cols === num ? 'bg-white text-black shadow-lg' : 'text-zinc-600'}`}>{num}</button>
              ))}
            </div>
          </section>
          ...
          <section className="bg-white/5 p-4 rounded-2xl border border-white/5">
            ...
            <input type="password" placeholder="Paste Personal Token..." className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-[10px] font-mono outline-none focus:border-blue-500/50 text-white" value={discogsToken} onChange={(e) => { localStorage.setItem('walkman_discogs_token', e.target.value); setDiscogsToken(e.target.value); }} />
          </section>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-black/20 rounded-2xl p-5 space-y-8">
                  <section>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['ALL', 'MAM', 'SZUKAM'].map(s => <button key={s} onClick={() => setDefaultStatus(s)} className={`py-2.5 rounded-lg text-[9px] font-black uppercase border transition-all ${defaultStatus === s ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-white/5 text-zinc-600'}`}>{s === 'SZUKAM' ? 'WISH' : s}</button>)}
                    </div>
                  </section>
                  <section>
                    <Label title="Default Format" />
                    <div className="grid grid-cols-4 gap-2">
                      {['ALL', 'FLAC', 'MP3', 'Hi-Res'].map(f => <button key={f} onClick={() => setDefaultFormat(f)} className={`py-2.5 rounded-lg text-[9px] font-black uppercase border transition-all ${defaultFormat === f ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-white/5 text-zinc-600'}`}>{f}</button>)}
                    </div>
                  </section>
                  <section>
                    <Label title="Default Sort Order" />
                    <div className="grid grid-cols-2 gap-2">
                      {[{id:'recent',l:'RECENT'},{id:'artist',l:'ARTIST A-Z'},{id:'album',l:'ALBUM A-Z'},{id:'year',l:'YEAR'}].map(opt => <button key={opt.id} onClick={() => setDefaultSort(opt.id)} className={`py-3 px-2 rounded-xl text-[9px] font-black uppercase border transition-all ${defaultSort === opt.id ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-white/5 text-zinc-600'}`}>{opt.l}</button>)}
                    </div>
                  </section>
                </motion.div>
              )}
