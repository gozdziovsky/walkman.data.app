...
function App() {
  ...
  // --- 1. KONFIGURACJA BIEŻĄCA (Tymczasowa) ---
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white pb-32">
      <header className="px-6 pt-12 space-y-6 text-center">
        ...
        <div className="flex items-center justify-between bg-zinc-900/40 backdrop-blur-md border border-white/5 rounded-[2rem] p-2 pl-6 shadow-2xl">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setShowFilters(true)} className={`p-4 rounded-full bg-zinc-900/50 border border-transparent text-zinc-500 hover:text-white transition-all active:scale-90 relative`}>
              <Filter size={18} />
              {activeFiltersCount > 0 && (
                <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-green-500 rounded-full border-2 border-[#09090b]" />
              )}
            </button>
      ...
      {/* FILTER DRAWER - Tylko dla bieżącego widoku */}
      <AnimatePresence>
        {showFilters && (
          <>
      ...
      {/* SETTINGS MODAL - Zapisuje DEFAULTS do localStorage */}
      {showSettings && (
        <SettingsModal 
          cols={cols} setCols={setCols} 
          defaultFormat={defaultFormat} setDefaultFormat={(val: string) => { localStorage.setItem('walkman_default_format', val); setDefaultFormat(val); }}
          defaultStatus={defaultStatus} setDefaultStatus={(val: string) => { localStorage.setItem('walkman_default_status', val); setDefaultStatus(val); }}
          defaultSort={defaultSort} setDefaultSort={(val: any) => { localStorage.setItem('walkman_default_sort', val); setDefaultSort(val); }}
...
// ... reszta komponentów (FilterBtn, SortBtn, FilterLabel) bez zmian
