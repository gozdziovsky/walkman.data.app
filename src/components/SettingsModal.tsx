        ...
        <div className="space-y-6">
          ...
          <section className="bg-white/5 p-4 rounded-2xl border border-white/5">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-[10px] font-mono outline-none focus:border-blue-500/50 text-white transition-all" 
              value={discogsToken} 
              onChange={(e) => { localStorage.setItem('walkman_discogs_token', e.target.value); setDiscogsToken(e.target.value); }} 
            />
          </section>

          {/* STARTUP DEFAULTS (COLLAPSIBLE) */}
          <div className="space-y-4">
            <button onClick={() => setIsStartupOpen(!isStartupOpen)} className="w-full flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-all">
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['ALL', 'MAM', 'SZUKAM'].map(s => (
                        <button key={s} onClick={() => updateDefault('walkman_default_status', s, setDefaultStatus)} className={`py-2.5 rounded-lg text-[9px] font-black uppercase border transition-all ${defaultStatus === s ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-white/5 text-zinc-600'}`}>
                          {s === 'SZUKAM' ? 'WISH' : s}
                        </button>
                      ))}
                    </div>
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {['ALL', 'FLAC', 'MP3', 'Hi-Res'].map(f => (
                        <button key={f} onClick={() => updateDefault('walkman_default_format', f, setDefaultFormat)} className={`py-2.5 rounded-lg text-[9px] font-black uppercase border transition-all ${defaultFormat === f ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-white/5 text-zinc-600'}`}>
                          {f}
                        </button>
                      ))}
                    </div>
