...
import { X, Play, MonitorPlay, Trash2, Edit3, Save, ImageIcon, Loader2, Calendar, Music, ListMusic, ChevronUp, ChevronDown, Star, Link2, Disc } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex items-end md:items-center justify-center p-0 md:p-6" onClick={onClose}>
      <motion.div className="bg-zinc-900 w-full max-w-5xl rounded-t-[2.5rem] md:rounded-[3rem] overflow-hidden flex flex-col md:flex-row max-h-[95vh] shadow-2xl relative" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full md:w-1/2 aspect-square relative bg-zinc-950 shrink-0 overflow-hidden">
          {!isEdit && album.tracks && (
            <div className="absolute inset-0 bg-zinc-950 p-8 overflow-y-auto no-scrollbar z-0">
               <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
                  ...
                  <button onClick={() => setShowTracks(false)} className="text-zinc-500 bg-white/5 p-2 rounded-full"><ChevronDown size={16} /></button>
               </div>
          ...
          >
            ...
            {isEdit && (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-8 gap-4">
                <button onClick={() => fileInputRef.current?.click()} className="bg-white text-black px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2"><ImageIcon size={16} /> Change Cover</button>
            ...
            {!isEdit && album.tracks && !showTracks && (
               <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center opacity-70 animate-bounce"><ChevronUp size={24} className="text-white" /><span className="text-[8px] font-black uppercase text-white mt-1">Tracks</span></div>
            )}
            {!showTracks && (
              <div className="absolute top-6 left-6 flex gap-3 z-20">
                {!isEdit ? (
                  <button onClick={() => setIsEdit(true)} className="p-4 bg-black/40 backdrop-blur-md rounded-full text-white"><Edit3 size={20} /></button>
                ) : (
                  <div className="bg-brand text-black px-4 py-2 rounded-full text-[10px] font-black uppercase">Editing</div>
                )}
              </div>
            )}
            {!showTracks && <button onClick={onClose} className="absolute top-6 right-6 p-4 bg-black/40 backdrop-blur-md rounded-full text-white z-20"><X size={20} /></button>}
          </motion.div>
        </div>
        ...
        <div className="p-8 md:p-12 flex-1 overflow-y-auto no-scrollbar text-white text-left">
          <AnimatePresence mode="wait">
            {isEdit ? (
              <motion.div key="edit" className="space-y-6 pb-10">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Artist</label><input className="w-full bg-white/5 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-brand/50" value={form.artist} onChange={e => setForm({...form, artist: e.target.value})} /></div>
                  <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Title</label><input className="w-full bg-white/5 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-brand/50" value={form.title} onChange={e => setForm({...form, title: e.target.value})} /></div>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-zinc-600 uppercase">Status</label>
                  <div className="grid grid-cols-2 gap-2 bg-black/40 p-1 rounded-xl h-12">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Year</label><input type="number" className="w-full bg-white/5 rounded-xl px-4 py-3 text-sm font-bold outline-none" value={form.year} onChange={e => setForm({...form, year: parseInt(e.target.value)})} /></div>
                  <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Format</label><select className="w-full bg-zinc-800 rounded-xl px-3 py-3 text-[10px] font-black uppercase" value={form.format} onChange={e => setForm({...form, format: e.target.value as any})}><option value="FLAC">FLAC</option><option value="MP3">MP3</option><option value="Hi-Res">Hi-Res</option></select></div>
                  <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Rating</label><input type="number" className="w-full bg-white/5 rounded-xl px-4 py-3 text-sm font-bold outline-none" value={form.rating} onChange={e => setForm({...form, rating: parseInt(e.target.value)})} /></div>
                </div>
                <div className="space-y-1"><label className="text-[9px] font-black text-zinc-600 uppercase">Tracks</label><textarea className="w-full bg-white/5 rounded-xl px-4 py-3 text-[11px] font-mono h-32 resize-none" value={form.tracks || ''} onChange={e => setForm({...form, tracks: e.target.value})} /></div>
                <div className="flex gap-4 pt-4"><button onClick={() => setIsEdit(false)} className="flex-1 py-4 bg-zinc-800 rounded-2xl text-[10px] font-black uppercase">Discard</button><button onClick={handleUpdate} disabled={loading} className="flex-[2] py-4 bg-brand text-black rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 shadow-lg shadow-brand/20">{loading ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Save</button></div>
              </motion.div>
            ) : (
              <div className="space-y-8">
                <div>
                  ...
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0 mt-2">
                      {album.rating > 0 && <div className="flex items-center gap-1 bg-brand text-black px-3 py-1 rounded-lg"><Star size={12} fill="black" /><span className="text-xs font-black">{album.rating}/10</span></div>}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-4">
                  <button onClick={() => window.open(`https://open.spotify.com/search/${encodeURIComponent(album.artist + " " + album.title)}`, '_blank')} className="py-5 bg-white text-black rounded-[1.8rem] font-black uppercase text-[10px] flex items-center justify-center gap-3">
                    <Play size={18} fill="black" /> Search Spotify
                  </button>
                  <button onClick={() => window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(album.artist + " " + album.title + " full album")}`, '_blank')} className="py-5 bg-zinc-800 text-white rounded-[1.8rem] font-black uppercase text-[10px] flex items-center justify-center gap-3">
