...
import { X, Play, MonitorPlay, Trash2, Loader2, Edit3, Check, Star, Calendar, Music, Disc } from 'lucide-react';
...
export const DetailsModal = ({ album, onClose, onUpdateSuccess }: { album: Album, onClose: () => void, onUpdateSuccess: () => void }) => {
  ...
  const update = async () => {
    try {
      await supabase.from('albums').update({ title: form.title, artist: form.artist, year: form.year, genre: form.genre, rating: form.rating, status: form.status }).eq('id', album.id);
            ...
            {isEdit ? (
              <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                ...
                <div className="flex gap-2">
                  {[1,2,3,4,5].map(s => (
                    <button key={s} onClick={() => setForm({...form, rating: s})} className={form.rating >= s ? 'text-yellow-500' : 'text-zinc-800'}><Star size={20} fill={form.rating >= s ? 'currentColor' : 'none'} /></button>
                  ))}
                </div>
              ...
              <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  <div className="px-4 py-2 bg-zinc-800 rounded-lg text-[9px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2"><Calendar size={12} /> {album.year}</div>
                  <div className="px-4 py-2 bg-zinc-800 rounded-lg text-[9px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2"><Music size={12} /> {album.genre}</div>
