      ...
      <motion.div layout className="bg-zinc-900 w-full max-w-4xl rounded-t-[2.5rem] md:rounded-[3.5rem] overflow-hidden border-t md:border border-white/10 flex flex-col md:flex-row max-h-[95vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative w-full md:w-[45%] aspect-square md:aspect-auto">
          ...
          <div className="absolute top-6 left-6 flex gap-2">
            <button onClick={() => setIsEdit(!isEdit)} className={`p-4 rounded-full backdrop-blur-md ${isEdit ? 'bg-green-500 text-black' : 'bg-black/40 text-white'}`}><Edit3 size={18} /></button>
            <button onClick={del} className="p-4 bg-red-500/20 text-red-500 rounded-full backdrop-blur-md">{isDeleting ? <Loader2 className="animate-spin" size={18} /> : <Trash2 size={18} />}</button>
          </div>
          <button onClick={onClose} className="absolute top-6 right-6 p-3 bg-black/40 text-white rounded-full"><X size={18} /></button>
        </div>
          ...
          <AnimatePresence mode="wait">
            {isEdit ? (
              <motion.div key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <input className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xl font-black uppercase text-white outline-none focus:border-green-500" value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
                ...
                <textarea className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs font-bold min-h-[100px]" value={form.tracks || ''} onChange={e => setForm({...form, tracks: e.target.value})} placeholder="Tracklist" />
              ...
              <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none mb-6">{album.title}</h2>
                ...
                {album.tracks && (
                  <div className="mb-10 bg-white/[0.02] rounded-2xl p-6 border border-white/5 text-[11px] font-mono text-zinc-400 whitespace-pre-line">
                    {album.tracks}
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => album.spotify_url && window.open(album.spotify_url, '_blank')} disabled={!album.spotify_url} className={`py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 ${album.spotify_url ? 'bg-white text-black' : 'bg-zinc-800 text-zinc-600 opacity-50'}`}>
                    <Play size={14} fill="currentColor" /> Spotify {album.spotify_url && <ExternalLink size={10} />}
                  </button>
                  <button onClick={() => album.youtube_url && window.open(album.youtube_url, '_blank')} disabled={!album.youtube_url} className={`py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2 border ${album.youtube_url ? 'bg-zinc-800 text-white border-white/10' : 'bg-zinc-800 text-zinc-600 border-transparent opacity-50'}`}>
